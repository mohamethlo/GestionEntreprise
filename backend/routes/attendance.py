...
from models import db, Attendance, WorkLocation, User

attendance_bp = Blueprint("attendance_bp", __name__)

# -------------------------
# Obtenir l'historique du pointage
# -------------------------
...
def check_out():
    ...
    if location_name != today_attendance.check_in_location:
        return jsonify({
            "success": False,
            "message": "Zone de sortie différente de l'entrée."
        }), 400
...
def today_stats():
    ...
    for user in users:
        ...
        if attendance and attendance.check_in:
            presents.append({"id": user.id, "name": user.name})
            if attendance.check_in.time() > heure_limite:
                retards.append({"id": user.id, "name": user.name})
        else:
            absents.append({"id": user.id, "name": user.name})
